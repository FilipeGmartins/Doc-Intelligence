import { FileCheck2, Files, LayoutDashboard, Menu, ScanText, Upload, X } from 'lucide-react'
  ...
  return (
    <div className="app-shell">
      <header className="mobile-header">
        <Brand />
        <button className="icon-button" type="button" aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} onClick={() => setMenuOpen((open) => !open)}>
      ...
      <aside className={`sidebar ${menuOpen ? 'sidebar--open' : ''}`}>
        ...
        <div className="sidebar-user" aria-label="Sessão demonstrativa">
...
function Brand() {
  return (
    <div className="brand">
      ...
      <span><strong>DOC</strong> Intelligence</span>
    </div>
  )
}
