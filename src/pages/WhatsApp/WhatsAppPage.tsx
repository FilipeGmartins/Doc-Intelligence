import { BadgeCheck, Bot, Check, Clock3, FileUp, Info, MessageCircleMore, RotateCcw, Send, UserRoundCheck } from 'lucide-react'
...
export function WhatsAppPage() {
  ...
  return (
    <div className="page whatsapp-page">
      ...
      <div className="automation-note"><Info size={17} aria-hidden="true" /><p><strong>Como testar:</strong> selecione Maria, responda às perguntas, simule o envio do documento e valide o pré-cadastro. O resultado aparecerá em Pessoas.</p></div>
        ...
        {selected ? <div className="intake-surface">
          ...
          <aside className="intake-draft" aria-label="Pré-cadastro extraído da conversa">
            ...
            <div className="intake-actions">
              ...
              {selected.status === 'awaiting_internal_review' ? <button className="primary-button primary-button--button" type="button" disabled={busy} onClick={() => void execute(() => approve(selected.id), 'Cadastro validado e incluído em Pessoas.')}><UserRoundCheck size={17} />Validar e criar pessoa</button> : null}
              {selected.status === 'approved' ? <div className="intake-approved"><Check size={17} /><span><strong>Cadastro criado</strong><small>A pessoa já está disponível na visão consolidada.</small></span></div> : null}
